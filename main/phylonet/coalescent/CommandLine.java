	...
	private static SimpleJSAP getJSAP() throws JSAPException {
		return new SimpleJSAP(
				...
				new Parameter[] {
						...
						new FlaggedOption("placement",
								JSAP.STRING_PARSER,
								null,
								JSAP.NOT_REQUIRED,
								JSAP.NO_SHORTFLAG,"placement", "Inserts the new species in the gene trees, whose name is provided with this option,"+
								" into the input species tree given by -f"),
...
//		placement = config.contains("placement") ? config.getBoolean("placement")
//				: false;
		String placementStr = config.contains("placement") ? config.getString("placement")
				: "";
		try {
	...
	public static void main(String[] args) throws Exception {
		...
		if (config.getFile("score species trees") != null) {
			...
			runInference(config, criterion, rooted, extrarooted, mainTrees,
					outbuffer, bootstrapInputSets, options, outgroup, extraTrees);
		}
	...
	private static void readExtraTrees(JSAPResult config, boolean extrarooted, List<Tree> extraTrees){
	...
	private static void runInference(JSAPResult config, int criterion,
			...
			Options options, String outgroup, List<Tree> extraTrees) throws JSAPException,
...
//		System.err.println("new species: "+((STINode) node).getData());
	...
	private static void readExtraInputTrees(List<Tree> trees, List<String> lines,
			boolean rooted, boolean checkCompleteness, boolean stLablel,
			Integer minleaves, int annotation, String outgroup)
			throws FileNotFoundException, IOException {
	...
	private static void readExtraInputTrees2(List<Tree> trees,boolean rooted, boolean stLabel) {
...
//					} 
	...
	private static void readAndPruneInputTrees(List<Tree> trees, List<String> lines,
			boolean rooted, boolean checkCompleteness, boolean stLablel,
			Integer minleaves, int annotation, String outgroup, boolean placement ,List<Tree> extraSpTtrees, String newSpeciesName)
			throws FileNotFoundException, IOException {
...
//					if(newSpeciesName.length() == 0){
//						gtLeaves = new ArrayList<String>(Arrays.asList(gt.getLeaves()));
...
//						System.err.println("pruned1");
					...
					MutableTree tr = nr.readTree();
//					if(newSpeciesName.length() == 0){
//						gtLeaves = new ArrayList<String>(Arrays.asList(tr.getLeaves()));
//						gtLeaves.removeAll(stLeaves);
//						newSpeciesName = gtLeaves.get(0);
//						stLeaves.add(newSpeciesName);							
//					}
